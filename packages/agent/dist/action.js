...
function asPage(container) {
    // Page has .frames(); Frame doesn’t
    return container.frames ? container : container.page();
}
// Resolve a Frame by index or by iframe selector
async function getFrame(container, iframe) {
    const page = asPage(container);
    if (typeof iframe === 'number') {
        const f = page.frames()[iframe];
        if (!f)
            throw new Error(`frame index not found: ${iframe}`);
        return f;
    }
    ...
    if (!frame)
        throw new Error(`no contentFrame for selector: ${iframe}`);
    return frame;
}
export async function selectDropdown(p, selector, valueOrLabel) {
...
export async function setCheckbox(p, selector, checked = true) {
    const sel = resolveSelector(selector);
    const el = await p.waitForSelector(sel, { timeout: 15000 });
...
export async function waitNetworkIdle(p, idleMs = 800, timeoutMs = 15000) {
    let inFlight = 0;
    let idleResolve;
    let idleTimer = null;
    const done = new Promise((res) => (idleResolve = res));
    const bump = () => {
        if (idleTimer)
